...
static void test_rangeproof_api(void) {
    ...
    uint64_t vmin = rustsecp256k1zkp_v0_11_0_testrand32();
    uint64_t val = vmin + rustsecp256k1zkp_v0_11_0_testrand32();
...
static void test_borromean(void) {
    ...
    size_t j;
    int c;
    rustsecp256k1zkp_v0_11_0_testrand256_test(m);
    nrings = 1 + (rustsecp256k1zkp_v0_11_0_testrand32()&7);
    c = 0;
    rustsecp256k1zkp_v0_11_0_scalar_set_int(&one, 1);
    if (rustsecp256k1zkp_v0_11_0_testrand32()&1) {
        rustsecp256k1zkp_v0_11_0_scalar_negate(&one, &one);
    }
    for (i = 0; i < nrings; i++) {
        rsizes[i] = 1 + (rustsecp256k1zkp_v0_11_0_testrand32()&7);
        secidx[i] = rustsecp256k1zkp_v0_11_0_testrand32() % rsizes[i];
        random_scalar_order(&sec[i]);
        random_scalar_order(&k[i]);
        if(rustsecp256k1zkp_v0_11_0_testrand32()&7) {
            sec[i] = one;
        }
        if(rustsecp256k1zkp_v0_11_0_testrand32()&7) {
            k[i] = one;
    ...
    i = rustsecp256k1zkp_v0_11_0_testrand32() % c;
...
static void test_rangeproof(void) {
    ...
    {
        /*Malleability test.*/
        v = rustsecp256k1zkp_v0_11_0_testrandi64(0, 255);
        CHECK(rustsecp256k1zkp_v0_11_0_pedersen_commit(CTX, &commit, blind, v, rustsecp256k1zkp_v0_11_0_generator_h));
        len = 5134;
    ...
    for (i = 0; i < (size_t) COUNT; i++) {
        int exp;
        int min_bits;
        v = rustsecp256k1zkp_v0_11_0_testrandi64(0, UINT64_MAX >> (rustsecp256k1zkp_v0_11_0_testrand32()&63));
        vmin = 0;
        if ((v < INT64_MAX) && (rustsecp256k1zkp_v0_11_0_testrand32()&1)) {
            vmin = rustsecp256k1zkp_v0_11_0_testrandi64(0, v);
        }
        rustsecp256k1zkp_v0_11_0_testrand256(blind);
        CHECK(rustsecp256k1zkp_v0_11_0_pedersen_commit(CTX, &commit, blind, v, rustsecp256k1zkp_v0_11_0_generator_h));
        len = 5134;
        exp = (int)rustsecp256k1zkp_v0_11_0_testrandi64(0,18)-(int)rustsecp256k1zkp_v0_11_0_testrandi64(0,18);
        if (exp < 0) {
            exp = -exp;
        }
        min_bits = (int)rustsecp256k1zkp_v0_11_0_testrandi64(0,64)-(int)rustsecp256k1zkp_v0_11_0_testrandi64(0,64);
        if (min_bits < 0) {
            min_bits = -min_bits;
        }
    ...
    for (j = 0; j < 3; j++) {
        for (i = 0; i < 96; i++) {
            rustsecp256k1zkp_v0_11_0_testrand256(&proof[i * 32]);
        }
        for (k = 0; k < 128; k += 3) {
            len = k;
        ...
        len = rustsecp256k1zkp_v0_11_0_testrandi64(0, 3072);
        CHECK(!rustsecp256k1zkp_v0_11_0_rangeproof_verify(CTX, &minv, &maxv, &commit2, proof, len, NULL, 0, rustsecp256k1zkp_v0_11_0_generator_h));
    }
}
...
static void test_rangeproof_null_blinder(void) {
    ...
    /* Rewinding with 3-bits works */
    {
        ...
        rustsecp256k1zkp_v0_11_0_testrand256(&msg[32]);
        rustsecp256k1zkp_v0_11_0_testrand256(&msg[64]);
        rustsecp256k1zkp_v0_11_0_testrand256(&msg[96]);
...
static void test_single_value_proof(uint64_t val) {
    ...
    rustsecp256k1zkp_v0_11_0_testrand256(blind);
    rustsecp256k1zkp_v0_11_0_testrand256(nonce);
    CHECK(rustsecp256k1zkp_v0_11_0_pedersen_commit(CTX, &commit, blind, val, rustsecp256k1zkp_v0_11_0_generator_h));
...
static void test_multiple_generators(void) {
    const size_t n_inputs = (rustsecp256k1zkp_v0_11_0_testrand32() % (MAX_N_GENS / 2)) + 1;
    const size_t n_outputs = (rustsecp256k1zkp_v0_11_0_testrand32() % (MAX_N_GENS / 2)) + 1;
    ...
    for (i = 0; i < n_outputs; i++) {
        value[n_inputs + i] = rustsecp256k1zkp_v0_11_0_testrandi64(0, INT64_MAX - total_value);
        total_value += value[n_inputs + i];
    }
    for (i = 0; i < n_inputs - 1; i++) {
        value[i] = rustsecp256k1zkp_v0_11_0_testrandi64(0, total_value);
        total_value -= value[i];
    }
    value[i] = total_value;
